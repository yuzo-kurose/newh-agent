...
export default function ConceptStudio({ brief, color, initialData, initialConfirmed, onChange }: Props) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      {/* フィードバック＆アクション（カード最上部・全幅。ブロック2・3と統一） */}
      ...
      {/* 下部：ステップナビ｜提案スペース（スマホでは縦積み） */}
      <div style={{ display: "flex", flexDirection: isMobile ? "column" : "row", gap: 14, alignItems: isMobile ? "stretch" : "flex-start" }}>
        {/* ステップ（スマホでは横スクロールのチップ列） */}
        <div style={{ width: isMobile ? "100%" : 220, flexShrink: 0, display: "flex", flexDirection: isMobile ? "row" : "column", gap: 6, overflowX: isMobile ? "auto" : "visible", paddingBottom: isMobile ? 2 : 0 }}>
          {CONCEPT_ELEMENTS.map((e, i) => {
            ...
            return (
              <button key={e.key} onClick={() => { setSelected(e.key); setShowHistory(false); }}
                style={{ textAlign: "left", padding: "10px 12px", borderRadius: 8, cursor: "pointer", background: isSel ? T.white : "transparent", border: `1px solid ${isSel ? color : T.border}`, display: "flex", flexDirection: "column", gap: 2, flexShrink: isMobile ? 0 : undefined, minWidth: isMobile ? 150 : undefined }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <span style={{ width: 18, height: 18, borderRadius: "50%", fontSize: 12, fontWeight: 800, display: "flex", alignItems: "center", justifyContent: "center", background: isDone ? T.green : isSel ? color : T.paper, color: isDone || isSel ? T.white : T.inkMuted, flexShrink: 0 }}>{isDone ? "✓" : i + 1}</span>
                ...
                {!isMobile && <span style={{ fontSize: 12, color: T.inkFaint, paddingLeft: 26 }}>{e.hint}</span>}
              </button>
            );
          })}
          {allConfirmed && !isMobile && (
        ...
        {/* 提案スペース */}
        <div style={{ flex: 1, minWidth: 0, border: `1px solid ${T.border}`, borderRadius: 10, background: T.white, padding: isMobile ? 12 : 16, display: "flex", flexDirection: "column", gap: 12 }}>
