  ...
  return (
    <nav style={{ width:collapsed?52:210, background:T.white, borderRight:`1px solid ${T.border}`, flexShrink:0, display:"flex", flexDirection:"column", transition:"width 0.2s ease", overflowX:"hidden" }}>
      <button onClick={onToggleCollapse}
        style={{ padding:"10px", background:"transparent", border:"none", borderBottom:`1px solid ${T.border}`, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:collapsed?"center":"flex-end", color:T.inkFaint, fontSize:16, flexShrink:0 }}
      ...
      <div style={{ overflowY:"auto", flex:1 }}>
        ...
        <button onClick={onOpenVds} title={collapsed ? "VDS設計" : ""}
          ...
          <span style={{ fontSize:12, color:activeView==="vds"?T.ink:T.inkFaint }}>◆</span>
          {!collapsed && <span style={{ fontSize:11, color:activeView==="vds"?T.ink:T.inkMuted, fontWeight:activeView==="vds"?700:400 }}>VDS設計</span>}
        </button>
        {phases.map((p, i) => {
            ...
            <button key={p.id} onClick={() => setActivePhase(i)} title={collapsed ? p.label : ""}
              ...
              {collapsed ? (
                <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:3 }}>
                  <span style={{ fontSize:14, color:active?p.band:T.inkFaint }}>{p.icon}</span>
                ...
                <>
                  <div style={{ display:"flex", alignItems:"center", gap:6 }}>
                    <span style={{ fontSize:11, color:active?p.band:T.inkFaint }}>{p.icon}</span>
                    <span style={{ fontSize:11, color:active?T.ink:T.inkMuted, fontWeight:active?700:400, lineHeight:1.3 }}>{p.label}</span>
                  </div>
                  <div style={{ display:"flex", alignItems:"center", gap:6, paddingLeft:17 }}>
                    ...
                    <span style={{ fontSize:9, color:T.inkFaint }}>{done}/{p.tasks.length}</span>
                  </div>
                </>
              )}
            </button>
