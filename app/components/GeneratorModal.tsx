...
- 各タスクの値は、currentHypothesis/missingInfo/discussionPoints/conclusionに分ける。
- currentHypothesisは現時点での仮説。
- missingInfoはタスク完了に向けて不足している情報。
- discussionPointsはチームやクライアントと議論すべき論点。
- conclusionは現時点の結論または次に確定すべき結論。
- 情報不足が大きい場合は、statusをneeds_infoにして、精度向上に効く追加質問を最大3件返す。
- ただし、分かる範囲の仮説は必ずhypothesesに入れる。
- 質問への回答がすでに与えられている場合は、statusをreadyにする。
- JSONのみ返す。前置き・説明・Markdownコードブロック不要。
- 各フィールドは60〜80字以内で簡潔に。長文は禁止。
- questionsのquestionとreasonも各50字以内。

JSON形式:
{"status":"needs_info|ready","summary":"案件理解の要約（80字以内）","questions":[{"id":"q1","question":"質問（50字以内）","reason":"理由（30字以内）"}],"hypotheses":{"setup-s1":{"currentHypothesis":"仮説（80字以内）","missingInfo":"不足情報（60字以内）","discussionPoints":"論点（60字以内）","conclusion":"結論（60字以内）"}}}`;
