...
収益性の4階層で採算を確認する（econ〜 に成否と根拠を書く）:
...
export const AGENTS: Record<string, AgentDef> = {
  ...
  revenue: {
    ...
    system: AGENT_BASE + NEWH_THINKING + BLOCK_THINKING.revenue + `\nVDSブロック3「収支モデル」を生成。前述の売上分解・コスト4分類・収益性4階層・事業成立の急所を内省した結果をJSONで返す。JSONのみ：\n{"unitPrice":"単価（誰からいくら）","customers":"客数（規模感）","period":"時間/期間（取引頻度・継続）","costValueDelivery":"価値提供コスト（自社事業の具体）","costScaleRealization":"規模実現コスト（自社事業の具体）","costMaintenance":"維持運営コスト（自社事業の具体）","costLaunchEnhancement":"立上げ/強化コスト（自社事業の具体）","econValue":"バリューエコノミクス：価値提供コストvs単価の成否と根拠","econUnit":"ユニットエコノミクス：LTVvsCACの成否と根拠","econBusiness":"ビジネスエコノミクス：損益分岐・固定費回収の成否と根拠","econInvest":"インベストエコノミクス：累積支出vs累積売上の成否と根拠","heavyCost":"valueDelivery|scaleRealization|maintenance|launchEnhancement","heavyCostGrowth":"ヘビーコストの増え方（何に連動して増えるか）","recoveryAxis":"回収軸（規模/時間 のどちらで回収するか・理由）","costPattern":"value|scale|maintenance|launch","keyPoint":"採算成立の急所（どことどこをバランスさせるか・一言）","recoveryEngine":"回収エンジン（何で投資を回収するか）","pricingModel":"料金モデル（誰から何に課金し収入を創るか・フロー/ストック）","costStructure":"コスト構造（4分類の要約）","profitability":"採算成立の目算（黒字化時期・UE）"}`,
