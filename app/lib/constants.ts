...
// ============================================================
// REVENUE (VDSブロック3) — 収支モデルの構造化スキーマ
// ============================================================
// コストの4分類（会計費目・増え方の特性つき）。
export const COST_CATEGORIES = [
  { key: "valueDelivery", label: "価値提供コスト", desc: "単価を得る/価値提供ごとに発生。変動費系原価（直接材料費・外注費・サーバー費等）＋変動費系販売費（包装費・物流費）。", growth: "取引・価値提供ごとに増える" },
  { key: "scaleRealization", label: "規模実現コスト", desc: "客数を増やすために発生。変動費系販売費（広告宣伝費・顧客獲得関連費）。", growth: "規模拡大で増える" },
  { key: "maintenance", label: "維持運営コスト", desc: "事業を維持し顧客を繋ぎ止めるために発生。一般管理費（人件費・賃料・管理費）＋固定費系原価（直接労務費・通信費・光熱費・CS費）。", growth: "時間軸で積み上がる" },
  { key: "launchEnhancement", label: "立上げ/強化コスト", desc: "事業を立ち上げる/強化するために発生。投資・開発経費（研究開発費等）。", growth: "立上げ・強化の局面で発生" },
] as const;
...
// コスト構造パターン（ヘビーコスト別の収益性の焦点）。
export const COST_PATTERNS = [
  { key: "value", label: "価値提供コストが重い", example: "仕入れが発生する業態・価値提供に人のオペが伴う事業（例：フードデリバリー）", focus: "バリューエコノミクス（価値提供コストと料金のバランス）" },
  { key: "scale", label: "規模実現コストが重い", example: "受注リードタイムが長い／リーチ難（医者・士業）／2サイド・マルチサイドのマッチング", focus: "継続関係の構築と顧客獲得コスト（CAC）の圧縮" },
  { key: "maintenance", label: "維持運営コストが重い", example: "箱もの（コワーキング）・在庫を持たないECプラットフォーム・デジタル完結ソリューション", focus: "ビジネスエコノミクス（固定費を賄う規模到達・固定費の圧縮）" },
  { key: "launch", label: "立上げ/強化コストが重い", example: "研究開発に多大な初期投資が要る製薬・大規模設備が要るエネルギー等の装置産業", focus: "インベストエコノミクス（多大な初期投資の回収）" },
] as const;
export type CostPatternKey = (typeof COST_PATTERNS)[number]["key"];

// 収益モデル：回収エンジン（3種）。
export const RECOVERY_ENGINES = [
  { key: "value", label: "提供価値エンジン", desc: "向き合う顧客がそのまま費用負担者・支払い者となり、提供価値自体が回収エンジンとなる。", example: "紙芝居の体験価値→参加料金・パンフレット販売" },
  { key: "adjacent", label: "隣接課題エンジン", desc: "向き合う顧客は買わず、回収エンジンとなる価値や課題をずらして売上を獲得する。", example: "紙芝居ではなく水飴・煎餅の駄菓子販売（娯楽→食/おやつへ課題をずらす）" },
  { key: "asset", label: "アセットエンジン", desc: "事業活動で蓄積されるアセットを活かして売上を獲得する。", example: "集まった子供たちというアセット→子供向け教育企業へ情報提供" },
] as const;
export type RecoveryEngineKey = (typeof RECOVERY_ENGINES)[number]["key"];

// 料金モデルの4要素。
export const PRICING_ELEMENTS = [
  { key: "payer", label: "誰が（支払い者）" },
  { key: "target", label: "何に対して（支払い対象）" },
  { key: "timing", label: "どのように（支払いタイミング）" },
  { key: "amount", label: "いくらで（支払い料金）" },
] as const;

// 料金モデルの6つの観点と問い（図12-12）。
export const PRICING_FIT_POINTS = [
  { key: "decision", label: "意思決定構造との整合", question: "意思決定者/費用負担者を特定し、費用負担で重視される要素・観点をおさえているか" },
  { key: "issue", label: "課題特性との整合", question: "支払い許容額を料金に反映し、課題の発生頻度と顧客の費用負担頻度が整合しているか" },
  { key: "strategy", label: "競争/戦略との整合", question: "フック（選ばれる理由）を体現し、ロック（選ばれ続ける理由）を阻害しないモデルか" },
  { key: "cost", label: "コスト構造との整合", question: "価値提供コストを上回る単価で、他コストを回収し成立を目指せる回収軸を考慮できているか" },
  { key: "value", label: "価値特性との整合", question: "価値実感と費用負担が相関する料金モデルか（実感が増すほど負担が増える）" },
  { key: "goal", label: "事業目標との整合", question: "目標到達に必要な顧客負担額と、育成フェーズで重視するスタンスに整合するか" },
] as const;
export type PricingFitKey = (typeof PRICING_FIT_POINTS)[number]["key"];
...
export interface RevenueResult {
  ...
  period: string;
  // 各階層の収入（アウトプット）側の見立て
  ltv: string; // ②収入：LTV（取引利益×継続期間）
  marginalProfit: string; // ③収入：限界利益・事業売上
  cumulativeRevenue: string; // ④収入：累積収益
  // コスト4分類（自社事業での具体）
  costValueDelivery: string;
  costScaleRealization: string;
  costMaintenance: string;
  costLaunchEnhancement: string;
  // 収益性の4階層（成立しているか）
  econValue: string;
  econUnit: string;
  econBusiness: string;
  econInvest: string;
  // 事業成立の急所
  heavyCost: CostCategoryKey; // ヘビーコストはどれか
  heavyCostGrowth: string; // 増え方の特性（何に連動するか）
  recoveryAxis: string; // 回収軸（規模/時間）
  costPattern: CostPatternKey; // コスト構造パターン
  keyPoint: string; // 採算成立の急所（どことどこをバランスさせるか）
  // 収益モデル：回収エンジン
  recoveryEngineType: RecoveryEngineKey; // 3種のどれか
  // 料金モデルの4要素
  pricePayer: string; // 誰が
  priceTarget: string; // 何に対して
  priceTiming: string; // どのように（タイミング）
  priceAmount: string; // いくらで
  // 料金モデルの6観点の整合
  fitDecision: string;
  fitIssue: string;
  fitStrategy: string;
  ...
  fitGoal: string;
  // 料金モデル設計思想
  designEmphasis: string; // 重視すべき観点
  designRequirement: string; // 料金モデルに求められること
  idealModel: string; // 理想的なモデル
  avoidModel: string; // 避けるべきモデル
  // VDS正本図スロット
...
- 仕組み（活動・機能・自社/パートナーリソース・チャネル）はブロック1のコンセプトと整合しているか。`,
  revenue: `
このブロック固有の思考プロセス（収支モデルを書く）:
収益性＝利益が出るか・収支の釣り合い。事業のコスト（インプット）を上回る売上（アウトプット）を生めるかの収支バランス。

売上＝単価×客数×時間/期間（unitPrice / customers / period に分けて書く）。

コストは4分類で捉える（自社事業の具体を cost〜 に書く）:
- 価値提供コスト：単価/価値提供ごとに発生（変動費系原価・販売費）。取引ごとに増える。
- 規模実現コスト：客数を増やすために発生（広告宣伝・顧客獲得費）。規模拡大で増える。
- 維持運営コスト：事業維持・顧客の繋ぎ止めに発生（一般管理費・固定費系原価）。時間軸で積み上がる。
- 立上げ/強化コスト：立ち上げ・強化に発生（投資・開発経費）。

各階層は「収入（アウトプット）と支出（インプット）のバランス」で捉える。収入側の見立ては unitPrice（①単価）/ ltv（②）/ marginalProfit（③限界利益・事業売上）/ cumulativeRevenue（④累積収益）に、支出側は cost〜 に書く。econ〜 には収支が釣り合うかの「結論（成否）」だけを一言で書き、収入・支出の説明を繰り返さない。

収益性の4階層で採算を確認する（収入＝上記/支出＝cost〜、結論＝econ〜）:
- バリューエコノミクス（価値単位）：価値提供コスト vs 単価。最ミクロ。ここが崩れると規模拡大・時間経過でも採算不可能。価値実現手法で提供コストが、顧客/課題・提供価値で支払い許容額が変わる。
- ユニットエコノミクス（顧客単位・時間軸）：LTV（月間取引単位利益×継続利用期間）vs CAC（顧客獲得費÷新規顧客数）。
- ビジネスエコノミクス（事業単位・規模）：限界利益で固定費を回収し単体黒字を出せるか＝損益分岐点。
- インベストエコノミクス（投資案件）：立上げ/強化込みの累積支出 vs 累積売上。投資に資するリターンを生めるか。

事業成立の急所（ヘビーコストを見抜き回収軸を掴む。3ステップ）:
1. ヘビーコスト把握：4分類のどれ/どの費目が総コストの大半か（heavyCost）。
2. 増え方を把握：何に連動して増えるか（利用者増・特定機能の利用・1人増・50人単位の階段型固定費 等。heavyCostGrowth）。
3. 回収軸：規模で増えるコストは時間経過で回収、時間で積み上がるコストは規模拡大で回収（recoveryAxis）。

コスト構造パターンで焦点を定める（costPattern）:
- 価値提供コストが重い（仕入れ・人オペ：フードデリバリー）→ バリューエコノミクスが焦点。
- 規模実現コストが重い（獲得が重い：長いリードタイム・医者/士業・2サイド/マルチサイド）→ 継続関係構築とCAC圧縮が焦点。
- 維持運営コストが重い（箱もの・ECプラットフォーム・デジタル完結）→ ビジネスエコノミクス（固定費を賄う規模到達・圧縮）が焦点。
- 立上げ/強化コストが重い（製薬R&D・装置産業）→ インベストエコノミクス（初期投資回収）が焦点。

keyPointには「どことどこをバランスさせるのが採算成立の急所か」を一言で書く。

収益モデル＝売上を獲得するモデル。「回収エンジン（何を売上の源泉とするか）」と「料金モデル（どう売上として回収するか）」で構成する。
回収エンジンは3種から選ぶ（recoveryEngineType＋recoveryEngineに説明）:
- 提供価値エンジン（value）：向き合う顧客がそのまま費用負担者となり、提供価値自体が回収源。
- 隣接課題エンジン（adjacent）：顧客は買わず、価値や課題をずらして回収（紙芝居→駄菓子）。
- アセットエンジン（asset）：蓄積アセットを活かして回収（集まった子供→教育企業へ）。
料金モデルは価格設定だけでなく4要素で規定する：pricePayer（誰が）/ priceTarget（何に対して）/ priceTiming（どのように・タイミング）/ priceAmount（いくらで）。pricingModelには全体像を一文で。
料金モデルを6観点で整合チェックする（fit〜 に成否・要点）:
- fitDecision（意思決定構造）：意思決定者/費用負担者を特定し、費用負担で重視される要素をおさえているか。
- fitIssue（課題特性）：支払い許容額を反映し、課題の発生頻度と費用負担頻度が整合しているか。
- fitStrategy（競争/戦略）：フックを体現し、ロックを阻害しないか（例：手軽さがフックなのに料金が分かりにくい／継続のためのデータ蓄積が重要なのに登録時課金で母数を減らす はNG）。
- fitCost（コスト構造）：価値提供コストを上回る単価か、他コストを回収し成立を目指せるか（バリューエコノミクスが原資）。
- fitValue（価値特性）：価値実感と費用負担が相関するか（実感が増すほど負担が増えるのが理想、逆は避ける）。主に「何に対して（支払対象）」に効く。
- fitGoal（事業目標）：目標到達に必要な顧客負担額・単価が適切か、育成フェーズのスタンス（獲得優先なら低単価/無料/広告、定着優先なら価値連動）と整合するか。
設計思想（図12-14。designEmphasis→designRequirement→idealModel／avoidModel）:
- designEmphasis：重視すべき観点（6観点のどれを最重視するか）。
- designRequirement：そこから料金モデルに求められること。
- idealModel：その要求を満たす理想的なモデル像。
- avoidModel：逆に避けるべき料金モデル像。

VDS図スロットとの対応：recoveryEngine＝回収エンジン、pricingModel＝料金モデル（4要素の要約）、costStructure＝コスト構造（4分類の要約）、profitability＝採算成立（4階層の結論）。各構造化フィールドと整合させる。`,
};
...
export const AGENTS: Record<string, AgentDef> = {
  ...
  revenue: {
    label: "ブロック3：収支モデル", icon: "¥", color: T.purple,
    reviewCriteria: "・売上が単価×客数×時間/期間で具体的に分解されているか\n・コストが4分類（価値提供/規模実現/維持運営/立上げ強化）で自社事業に即して捉えられているか\n・収益性の4階層（バリュー/ユニット/ビジネス/インベスト）で採算の成否が根拠とともに語られ、特にバリューエコノミクスが成立しているか\n・ヘビーコストが特定され、その増え方の特性と回収軸（規模/時間）が噛み合っているか\n・コスト構造パターンに応じた収益性の焦点（急所）が言えているか\n・回収エンジンが3種（提供価値/隣接課題/アセット）で明確で、料金モデルが4要素（誰が/何に/どのように/いくらで）で具体化されているか\n・料金モデルが6観点（意思決定構造/課題特性/競争・戦略/コスト構造/価値特性/事業目標）と整合し、フックを体現・ロックを阻害しないか\n・設計思想（重視観点→求められること→理想/避けるべきモデル）が一貫しているか",
    system: AGENT_BASE + NEWH_THINKING + BLOCK_THINKING.revenue + `\nVDSブロック3「収支モデル」を生成。前述の売上分解・コスト4分類・収益性4階層・事業成立の急所を内省した結果をJSONで返す。JSONのみ：\n{"unitPrice":"①収入：単価（誰からいくら）","customers":"客数（規模感）","period":"時間/期間（取引頻度・継続）","ltv":"②収入：LTV（取引利益×継続期間）の見立て","marginalProfit":"③収入：限界利益・事業売上の見立て","cumulativeRevenue":"④収入：累積収益の見立て","costValueDelivery":"①支出：価値提供コスト（自社事業の具体）","costScaleRealization":"②支出：規模実現コスト/CAC（自社事業の具体）","costMaintenance":"③支出：維持運営コスト/固定費（自社事業の具体）","costLaunchEnhancement":"④支出：立上げ/強化コスト（自社事業の具体）","econValue":"①結論：価値提供コストvs単価が釣り合うかの成否を一言（収入・支出の再説明はしない）","econUnit":"②結論：LTVvsCACが釣り合うかの成否を一言","econBusiness":"③結論：限界利益で固定費を回収できるかの成否を一言","econInvest":"④結論：累積収益が累積支出を上回るかの成否を一言","heavyCost":"valueDelivery|scaleRealization|maintenance|launchEnhancement","heavyCostGrowth":"ヘビーコストの増え方（何に連動して増えるか）","recoveryAxis":"回収軸（規模/時間 のどちらで回収するか・理由）","costPattern":"value|scale|maintenance|launch","keyPoint":"採算成立の急所（どことどこをバランスさせるか・一言）","recoveryEngineType":"value|adjacent|asset","pricePayer":"誰が（支払い者）","priceTarget":"何に対して（支払い対象）","priceTiming":"どのように（支払いタイミング）","priceAmount":"いくらで（支払い料金）","fitDecision":"意思決定構造との整合（成否・要点）","fitIssue":"課題特性との整合（支払い許容額・発生頻度）","fitStrategy":"競争/戦略との整合（フック体現・ロック非阻害）","fitCost":"コスト構造との整合（価値提供コストを上回る単価か）","fitValue":"価値特性との整合（価値実感と費用負担の相関）","fitGoal":"事業目標との整合（必要単価・育成フェーズのスタンス）","designEmphasis":"重視すべき観点","designRequirement":"料金モデルに求められること","idealModel":"理想的な料金モデル像","avoidModel":"避けるべき料金モデル像","recoveryEngine":"回収エンジン（何を売上の源泉とするか）","pricingModel":"料金モデル（4要素の要約）","costStructure":"コスト構造（4分類の要約）","profitability":"採算成立の目算（黒字化時期・UE）"}`,
