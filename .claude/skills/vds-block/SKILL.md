...
| revenue | 売上＝単価×客数×時間／コスト4分類（価値提供/規模実現/維持運営/立上げ強化＋会計費目・増え方）／収益性4階層（バリュー/ユニット/ビジネス/インベスト）／事業成立の急所（ヘビーコスト→増え方→回収軸、コスト構造4パターン）。表示は VdsTab の RevenueView | `COST_CATEGORIES` `ECONOMICS_LAYERS` `COST_PATTERNS` |
