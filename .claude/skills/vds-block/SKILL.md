...
- **出力が増えたら必ず [app/lib/generate.ts](../../../app/lib/generate.ts) の `MAX_TOKENS[blockId]` を見直す**（これを忘れると本番で出力が途中で切れ、JSONパースエラーになる）。
  - 目安：フィールドが増えるほど、また**フラットな（ネストの浅い）スキーマほど打ち切りに弱い**。`revenue` は約40フィールドのフラット構成で `7000` では切れたため `10000` に引き上げた実績あり。スキーマ例の文字数 × 想定値長を見て、1フィールド追加ごとに数百トークンの余裕を上乗せする感覚で。
  - 安全網として [app/lib/json.ts](../../../app/lib/json.ts) の `extractJSONText` が「途中で切れたJSON」を自動修復する（開いた文字列・括弧を閉じ、末尾の不完全トークンを捨てる）。ただし**修復は最後の保険**であり、欠けたフィールドは復元できない。`MAX_TOKENS` を適切に確保することが本筋。
